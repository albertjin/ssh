...
func dh(c *ssh, k *kexres, C *C) os.Error {
	switch k.Kex {
...
func dhWith(c *ssh, k *kexres, C *C, prime, gen *big.Int) os.Error {
	...
	if e!=nil || code!=msgNewkeys { panic("Expected msgNewkeys") }
	...
	c.wh = hmac.NewSHA1(hash('E'))
	c.rh = hmac.NewSHA1(hash('F'))
	return nil
}
